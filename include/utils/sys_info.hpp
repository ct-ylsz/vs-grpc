//
// Created by ct on 2021/12/26.
//

#ifndef HIBEAT_SYS_INFO_H
#define HIBEAT_SYS_INFO_H
...
inline float GetMemoryTotal(int pid) {
#ifdef WIN32
    MEMORYSTATUSEX m;
...
inline float GetMemoryUsage(int pid) {
   ...
   /* uint64_t mem = 0, vmem = 0;
    ...
    GlobalMemoryStatusEx(&m);
    // convert mem from B to MB
    return m.dwMemoryLoad / 1024.0 / 1024.0 ;
    // use GetCurrentProcess() can get current process and no need to close handle
